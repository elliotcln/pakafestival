    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if (!in_array('menu-item-has-children', $classes)) {
            ...
            if (!$has_top_level_page) {
                ...
                $output .= "<button aria-haspopup='true' aria-expanded='false' aria-owns='list-$this->parentItemCount' class='submenu-toggle'><span>$item->title";
                // the data-before attribute let's you change the value via js like with an event handler.
                $output .= "</span><svg class='icon' aria-hidden='true'>
                ...
                $output .= "</a>
        <button class='nav-submenu__button submenu-toggle' aria-haspopup='true' aria-expanded='false' aria-label='show submenu' aria-owns='list-$this->parentItemCount'>
        <svg class='icon'>
        <use xlink:href='#chevron-down'></use>
        </svg>
        </button>";
